...
Two mocked layers, a plain unit layer, and a config guard, ~1s combined for 94 tests at 100% coverage, plus one opt-in live test that runs against the real model:
...
- **End-to-end tests** (`TestStreamingClickPath`) — drive the real script via `streamlit.testing.v1.AppTest` with only `mlx_lm` mocked. Reaches branches the import-time tests can't: streaming click path and the settled `st.text` re-render after it, model-load failure, runtime target filtering, swap-button wiring, empty-text warning.
