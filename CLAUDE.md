...
Two mocked layers, a plain unit layer, and a config guard, ~1.5s combined for 105 tests at 100% coverage, plus one opt-in live test that runs against the real model:
...
Because the app catches generation failures and renders `st.error`, the live test asserts on `at.error` as well as `at.exception`; checking only the latter misreports a caught failure as an empty translation — the `wired_limit()` signature the test looks for next — and the real cause is lost.
...
- `app_test` — AppTest pre-run to its settled state, with `not at.exception` asserted after that first run
