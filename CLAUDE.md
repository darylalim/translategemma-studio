...
Two mocked layers, a plain unit layer, and a config guard, ~1s combined for 97 tests at 100% coverage, plus one opt-in live test that runs against the real model:
...
`assets/screenshot-dark.png` is the README's only screenshot: 2400×1250, a 1200×625 viewport at `device_scale_factor=2`. There is no capture script in the repo — this section is the recipe, because every step of it is a trap.
...
with sync_playwright() as p:
    ...
    ctx = browser.new_context(
        viewport={"width": 1200, "height": 625},
...
- **Re-measure the height after any layout change.** The viewport height is tuned to end ~42 CSS px below the buttons (their bottom is at 582.4 CSS px in the screenshot state); it is not a stable constant. Dropping the Material theme grew the page by ~45 CSS px at the buttons (its `baseFontSize = 14` and 36px title against Streamlit's 16 and 2.75rem), the Native theme then shrank it by ~14 (a 2rem title against the 2.75rem default), and removing the live token caption (2026-09-12) took out one caption row plus its gap (~38), which is why the height went 676 → 663 → 625. Measure the buttons' `getBoundingClientRect().bottom` at 1200 wide before trusting the number.
